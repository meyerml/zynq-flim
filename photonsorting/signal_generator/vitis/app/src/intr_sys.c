...
int addWarningsToInterruptSystem(){

	int status;

	// Connect the device driver handler
	status = XScuGic_Connect(p_XScuGicInst,
							PREMATURE_PIXEL_DONE_INTR_ID,
							(Xil_ExceptionHandler) PrematurePixelDoneIntrHandler,
							NULL
							);
					//		0U);
	if (status != XST_SUCCESS)
	...
	status = XScuGic_Connect(p_XScuGicInst,
							BIN_FULL_INTR_ID,
							(Xil_ExceptionHandler) BinFullIntrHandler,
							NULL
							);
					//		0U);
	if (status != XST_SUCCESS)
	...
	// Connect the device driver handler
	status = XScuGic_Connect(p_XScuGicInst,
							BIN_WRITTEN_BUT_NEVER_READ_INTR_ID,
							(Xil_ExceptionHandler) BinWrittenButNeverReadIntrHandler,
							NULL
							);
					//		0U);
	if (status != XST_SUCCESS)
	...
	// Connect the device driver handler
	status = XScuGic_Connect(p_XScuGicInst,
							FIFO_OVERFLOW_INTR_ID,
							(Xil_ExceptionHandler) FifoOverflowIntrHandler,
							NULL
							);
					//		0U);
	if (status != XST_SUCCESS)
	...
	XScuGic_SetPriorityTriggerType(p_XScuGicInst,
									PREMATURE_PIXEL_DONE_INTR_ID,
									PREMATURE_PIXEL_DONE_INTR_PRI,
									PREMATURE_PIXEL_DONE_INTR_TRIG);
	...
	XScuGic_SetPriorityTriggerType(p_XScuGicInst,
									BIN_WRITTEN_BUT_NEVER_READ_INTR_ID,
									BIN_WRITTEN_BUT_NEVER_READ_INTR_PRI,
									BIN_WRITTEN_BUT_NEVER_READ_INTR_TRIG);
	XScuGic_SetPriorityTriggerType(p_XScuGicInst,
									FIFO_OVERFLOW_INTR_ID,
									FIFO_OVERFLOW_INTR_PRI,
									FIFO_OVERFLOW_INTR_TRIG);
	/* Enable the interrupt */
	XScuGic_Enable(p_XScuGicInst, PREMATURE_PIXEL_DONE_INTR_ID);
	XScuGic_Enable(p_XScuGicInst, BIN_FULL_INTR_ID);
	XScuGic_Enable(p_XScuGicInst, BIN_WRITTEN_BUT_NEVER_READ_INTR_ID);
	XScuGic_Enable(p_XScuGicInst, FIFO_OVERFLOW_INTR_ID);
...
int addFrameToInterruptSystem(){
	...
	status = XScuGic_Connect(p_XScuGicInst,
							FRAME_INTR_ID,
							(Xil_ExceptionHandler) FrameIntrHandler,
							NULL
							);
					//		0U);
	if (status != XST_SUCCESS)
	...
	/* Enable the interrupt */
	XScuGic_Enable(p_XScuGicInst, FRAME_INTR_ID);
...
void PrematurePixelDoneIntrHandler(){
	xil_printf("Error: A premature_pixel_done_error was detected. "
			"This means that a pixel_done signal was sent before "
			"the histogram module had time to write the last pixel's "
			"histogram to DDR memory. consider increasing PULSES_PER_PIXEL "
			"or increasing log_2_bin_width.\r\n");
	while(1);
}

void BinFullIntrHandler(){
	xil_printf("Warning: A histogram bin has hit its limit."
			"The bin will saturate to its maximum value."
			"Consider decreasing log_2_bin_width or decreasing PULSES_PER_PIXEL.\r\n");
}
void BinWrittenButNeverReadIntrHandler(){
	xil_printf("Warning: A histogram bin is being filled, but it is never being read to DDR memory."
			"consider tweaking the LOG_2_HIST_BIN_WIDTH parameter.\r\n");
}

void FifoOverflowIntrHandler(){
	xil_printf("Warning: the LVDS modules axi stream fifo has overflown!\r\n");
}
