...
/******************************************************************************
* AXI CSN1 Channel and Pin Mapping
*
...
static XGpio 		XCSN1Inst;
static XGpio 		*p_XCSN1Inst = &XCSN1Inst;
...
int axiCSN1Init(void)
{
	 ...
	 * -------------------------------------------------------------------- */
	p_XCSN1Cfg = XGpio_LookupConfig(AXI_CSN1_DEVICE_ID);
	 if (p_XCSN1Cfg == NULL)
	 {
		 status = XST_FAILURE;
		 return status;
	  ...
	  * -------------------------------------------------------------------- */
	 status = XGpio_CfgInitialize(p_XCSN1Inst, p_XCSN1Cfg, p_XCSN1Cfg->BaseAddress);
	...
	* ------------ STEP 3: SELF TEST ------------
	* -------------------------------------------------------------------- */
	status = XGpio_SelfTest(p_XCSN1Inst);
	...
	* -------------------------------------------------------------------- */
	...
	XGpio_SetDataDirection(p_XCSN1Inst, AXI_CSN1_OP_CHANNEL, 0);


	/* CS initally not selected */
	axiCSN1OutSet(CSN1);
...
/*****************************************************************************
 * Function: axiCSN1OutSet()
 *//**
 *
 * @brief		Sets an AXI GPIO output pin.
 *
 * @param[in]	AxiCSN1_OutPin_t pin: The AXI channel(1) pin to set.
 *
 * @return		None
 *
...
void axiCSN1OutSet(AxiCSN1_OutPin_t pin){

	/* Function should only be passed GPIO bits 0-7 */
	Xil_AssertVoid(pin < 8);

	XGpio_DiscreteSet(p_XCSN1Inst, AXI_CSN1_OP_CHANNEL, (1 << pin));
...
/*****************************************************************************
 * Function: axiCSN1OutClear()
 *//**
 *
 * @brief		Clears an AXI GPIO output pin.
 *
 * @param[in]	AxiCSN1_OutPin_t pin		The AXI channel(1) pin to clear.
 *
 * @return		None
 *
 * @note		Assert functionality: Only accept output channel bits [7:0];
...
void axiCSN1OutClear(AxiCSN1_OutPin_t pin){

	/* Function should only be passed GPIO bits 0-7 */
	Xil_AssertVoid(pin < 8);

	XGpio_DiscreteClear(p_XCSN1Inst, AXI_CSN1_OP_CHANNEL, (1 << pin));
...
/*****************************************************************************
 * Function: axiCSN1OutToggle()
*//**
 *
 * @brief		Toggles an AXI GPIO output pin.
 *
 * @param[in]	AxiCSN1_OutPin_t pin		The AXI channel(1) pin to toggle.
 *
 * @return		None
 *
 * @note		Assert functionality: Only accept output channel bits [7:0];
...
void axiCSN1OutToggle(AxiCSN1_OutPin_t pin){
	...
	pin_state = (XGpio_DiscreteRead(p_XCSN1Inst, AXI_CSN1_OP_CHANNEL)
	 				& (1 << pin)) != 0;
