...
/******************************************************************************
* AXI SpiSelect Channel and Pin Mapping
*
...
static XGpio 		XSpiSelectInst;
static XGpio 		*p_XSpiSelectInst = &XSpiSelectInst;
...
int axiSpiSelectInit(void)
{
	...
	XGpio_Config *p_XSpiSelectCfg = NULL;
	 ...
	 * -------------------------------------------------------------------- */
	p_XSpiSelectCfg = XGpio_LookupConfig(AXI_SpiSelect_DEVICE_ID);
	 if (p_XSpiSelectCfg == NULL)
	 {
		 status = XST_FAILURE;
		 return status;
	 }
	  ...
	  * -------------------------------------------------------------------- */
	 status = XGpio_CfgInitialize(p_XSpiSelectInst, p_XSpiSelectCfg, p_XSpiSelectCfg->BaseAddress);
	...
	* ------------ STEP 3: SELF TEST ------------
	* -------------------------------------------------------------------- */
	status = XGpio_SelfTest(p_XSpiSelectInst);
	...
	* -------------------------------------------------------------------- */
	...
	XGpio_SetDataDirection(p_XSpiSelectInst, AXI_SpiSelect_OP_CHANNEL, 0);


	/* defualt is the defualt spi module */
	axiSpiSelectOutClear(SPI_SELECT_1);
 ...
 *
 * @param[in]	AxiSpiSelect_OutPin_t pin: The AXI channel(1) pin to set.
 *
 * @return		None
 *
...
void axiSpiSelectOutSet(AxiSpiSelect_OutPin_t pin){
	...
	XGpio_DiscreteSet(p_XSpiSelectInst, AXI_SpiSelect_OP_CHANNEL, (1 << pin));
...
/*****************************************************************************
 * Function: axiSpiSelectOutClear()
 *//**
 *
 * @brief		Clears an AXI GPIO output pin.
 *
 * @param[in]	AxiSpiSelect_OutPin_t pin		The AXI channel(1) pin to clear.
 *
 * @return		None
 *
 * @note		Assert functionality: Only accept output channel bits [7:0];
...
void axiSpiSelectOutClear(AxiSpiSelect_OutPin_t pin){
	...
	XGpio_DiscreteClear(p_XSpiSelectInst, AXI_SpiSelect_OP_CHANNEL, (1 << pin));
...
/*****************************************************************************
 * Function: axiSpiSelectOutToggle()
*//**
 *
 * @brief		Toggles an AXI GPIO output pin.
 *
 * @param[in]	AxiSpiSelect_OutPin_t pin		The AXI channel(1) pin to toggle.
 *
 * @return		None
 *
 * @note		Assert functionality: Only accept output channel bits [7:0];
...
void axiSpiSelectOutToggle(AxiSpiSelect_OutPin_t pin){
	...
	pin_state = (XGpio_DiscreteRead(p_XSpiSelectInst, AXI_SpiSelect_OP_CHANNEL)
	 				& (1 << pin)) != 0;

	if (pin_state == 1)
	{
		axiSpiSelectOutClear(pin);
	}
	else
	{
		axiSpiSelectOutSet(pin);
	}
}
