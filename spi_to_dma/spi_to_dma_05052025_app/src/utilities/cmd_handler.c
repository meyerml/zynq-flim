...
void executeCommand(uint8_t *tx_buffer) {
	...
	// --------------------------------------------------------------------------------- //
	// GPX2_READ_INTR_STATUS: Read back GPX2 interrupt status register
	// Field 1 = n/a, Field 2 = n/a
	// --------------------------------------------------------------------------------- //
	case GPX2_READ_INTR_STATUS:
		/* Write the data and update the response buffer */
		gpx2_intr_status = gpx2_ReadIntrStatus();
		setResponseBytes(tx_buffer, gpx2_intr_status);
		break;
