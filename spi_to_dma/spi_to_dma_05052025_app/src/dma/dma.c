...
//debug
	...
	/* Send a packet */
		...
		Xil_DCacheInvalidateRange(0x40400058, 4);
		int rlen3 = (XAxiDma_ReadReg((&AxiDma)->RegBase + \
						 (XAXIDMA_RX_OFFSET * XAXIDMA_DEVICE_TO_DMA), XAXIDMA_BUFFLEN_OFFSET));
		Xil_Out32(0x40400058, 0x00000010);
		Xil_DCacheFlushRange(0x40400058, 4);
		int rlen4 = (XAxiDma_ReadReg((&AxiDma)->RegBase + \
						 (XAXIDMA_RX_OFFSET * XAXIDMA_DEVICE_TO_DMA), XAXIDMA_BUFFLEN_OFFSET));
