...
/******************************************************************************
* AXI AxisOutEn Channel and Pin Mapping
*
...
static XGpio 		XAxisOutEnInst;
static XGpio 		*p_XAxisOutEnInst = &XAxisOutEnInst;
...
int axiAxisOutEnInit(void)
{
	int status;

	/* Pointer to XGpio_Config is required for later functions. */
	XGpio_Config *p_XAxisOutEnCfg = NULL;
	 ...
	 * -------------------------------------------------------------------- */
	p_XAxisOutEnCfg = XGpio_LookupConfig(AXI_AxisOutEn_DEVICE_ID);
	 if (p_XAxisOutEnCfg == NULL)
	 {
		 status = XST_FAILURE;
		 return status;
	 }
	  ...
	  * -------------------------------------------------------------------- */
	 status = XGpio_CfgInitialize(p_XAxisOutEnInst, p_XAxisOutEnCfg, p_XAxisOutEnCfg->BaseAddress);
	...
	* ------------ STEP 3: SELF TEST ------------
	* -------------------------------------------------------------------- */
	status = XGpio_SelfTest(p_XAxisOutEnInst);
	...
	* -------------------------------------------------------------------- */
	...
	XGpio_SetDataDirection(p_XAxisOutEnInst, AXI_AxisOutEn_OP_CHANNEL, 0);


	/* defualt is the defualt spi module */
	axiAxisOutEnOutClear(AXIS_OUT_EN);
 ...
 *
 * @param[in]	AxiAxisOutEn_OutPin_t pin: The AXI channel(1) pin to set.
 *
 * @return		None
 *
...
void axiAxisOutEnOutSet(AxiAxisOutEn_OutPin_t pin){

	/* Function should only be passed GPIO bits 0-7 */
	Xil_AssertVoid(pin < 8);

	XGpio_DiscreteSet(p_XAxisOutEnInst, AXI_AxisOutEn_OP_CHANNEL, (1 << pin));
...
/*****************************************************************************
 * Function: axiAxisOutEnOutClear()
 *//**
 *
 * @brief		Clears an AXI GPIO output pin.
 *
 * @param[in]	AxiAxisOutEn_OutPin_t pin		The AXI channel(1) pin to clear.
 *
 * @return		None
 *
 * @note		Assert functionality: Only accept output channel bits [7:0];
...
void axiAxisOutEnOutClear(AxiAxisOutEn_OutPin_t pin){

	/* Function should only be passed GPIO bits 0-7 */
	Xil_AssertVoid(pin < 8);

	XGpio_DiscreteClear(p_XAxisOutEnInst, AXI_AxisOutEn_OP_CHANNEL, (1 << pin));
...
/*****************************************************************************
 * Function: axiAxisOutEnOutToggle()
*//**
 *
 * @brief		Toggles an AXI GPIO output pin.
 *
 * @param[in]	AxiAxisOutEn_OutPin_t pin		The AXI channel(1) pin to toggle.
 *
 * @return		None
 *
 * @note		Assert functionality: Only accept output channel bits [7:0];
...
void axiAxisOutEnOutToggle(AxiAxisOutEn_OutPin_t pin){
	...
	pin_state = (XGpio_DiscreteRead(p_XAxisOutEnInst, AXI_AxisOutEn_OP_CHANNEL)
	 				& (1 << pin)) != 0;

	if (pin_state == 1)
	{
		axiAxisOutEnOutClear(pin);
	}
	else
	{
		axiAxisOutEnOutSet(pin);
	}
}
