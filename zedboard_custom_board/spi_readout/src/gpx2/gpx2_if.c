...
int gpx2_Init()
{
	...
	uint8_t reg4 = (refclock_divisions >> 0) & 0xFF;   // Least significant byte
	uint8_t reg5 = (refclock_divisions >> 8) & 0xFF;   // Middle byte
	uint8_t reg6 = (refclock_divisions >> 16) & 0x0F;  // Most significant byte of the three


	activechannels = STOP4ACTIVE;
/*
...
*/
	...
	uint8_t lvds_output = DISABLED;
	uint8_t reference_clock = REFCLK_ENABLED;
	uint8_t reg1 = reset_index  |  disable_pin | lvds_output | reference_clock | activechannels;

	//gpx2 configuration register 2
	uint8_t resolution = HIGH_RESOLUTION2;
	uint8_t channel_combine = NO_COMBINE;
	uint8_t reg2 = resolution | channel_combine | activechannels;

	//gpx2 configuration register 3
	uint8_t reg3 = 0x9F;
	...
	reg3 |= blockwise_fifo_read | common_fifo_read;

	uint8_t GPX2_STARTUP_CONFIG[17] = {reg1, reg2, reg3, reg4, reg5, reg6, 0xC0,
