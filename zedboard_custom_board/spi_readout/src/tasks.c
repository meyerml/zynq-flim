...
void task2(u32 *global_destination, u32 read_samples, u8 print_results, u8 print_statistics){
	...
	uint8_t* byte_pointer = (uint8_t*)global_destination +   //start of results_buffer
						...
						task_2_count*read_samples*6;					//already written
	...
	if (DMADone){
		...
		Xil_DCacheInvalidateRange((UINTPTR)global_destination, read_samples*6);
		...
		if (samples_so_far <= NUMSAMPLES-2){
		if (samples_so_far == 0){
		xil_printf("\r\nGOTOCSV\r\n");
		xil_printf("\r\nSTOPRESULTS:\r\n");
		}
