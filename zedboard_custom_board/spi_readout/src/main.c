...
uint32_t global_destination[DESTINATIONLENGTH];
...
//    destination[7] = 0xDEADC0DE;
	clear_ram_section((UINTPTR)global_destination, DESTINATIONLENGTH);
	...
	for(;;) // Infinite loop
	{
		...
		case TASK2:
			...
			{
				task2(global_destination, READ_SAMPLES_PER_CALL, PRINT_RESULTS, PRINT_STATISTICS);
				task2_complete = 1U;
				state = SERVICE_WDT;
			}
			break;
