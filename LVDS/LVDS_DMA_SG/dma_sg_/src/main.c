...
//    destination[7] = 0xDEADC0DE;
	...
	int Status = RxSetup((UINTPTR) &global_destination);
	...
	Status = ReportDataPerPhoton(global_destination, PRINT_REFINDICES, PRINT_STOPRESULTS, PRINT_STATISTICS);
	if (Status != XST_SUCCESS) {
		xil_printf("Reporting Data failed\r\n");
	}
	Status = ReportHistPerPixel(global_destination, PULSES_PER_PIXEL, PIXELS_IN_TOTAL, HIST_BIN_WIDTH);
	if (Status != XST_SUCCESS) {
