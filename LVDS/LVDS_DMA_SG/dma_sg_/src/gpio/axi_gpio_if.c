...
static XGpio 		*p_XGpioInst = &XGpioInst;
...
int axiGpioInit(void)
{
	 ...
	 * -------------------------------------------------------------------- */
	p_XGpioCfg = XGpio_LookupConfig(AXI_GPIO_DEVICE_ID);
	 if (p_XGpioCfg == NULL)
	 {
		 status = XST_FAILURE;
		 return status;
	  ...
	  * -------------------------------------------------------------------- */
	 status = XGpio_CfgInitialize(p_XGpioInst, p_XGpioCfg, p_XGpioCfg->BaseAddress);
	...
	* ------------ STEP 3: SELF TEST ------------
	* -------------------------------------------------------------------- */
	status = XGpio_SelfTest(p_XGpioInst);
	...
	* -------------------------------------------------------------------- */
	...
	XGpio_SetDataDirection(p_XGpioInst, AXI_GPIO_OP_CHANNEL, 0);
...
void axiGpOutToggle(AxiGpio_OutPin_t pin){
	...
	pin_state = (XGpio_DiscreteRead(p_XGpioInst, AXI_GPIO_OP_CHANNEL)
	 				& (1 << pin)) != 0;
...
uint32_t axiGpInRead(AxiGpio_InPin_t pin){
	...
	pin_state = (XGpio_DiscreteRead(p_XGpioInst, AXI_GPIO_IP_CHANNEL)
					& (1 << pin)) != 0;
