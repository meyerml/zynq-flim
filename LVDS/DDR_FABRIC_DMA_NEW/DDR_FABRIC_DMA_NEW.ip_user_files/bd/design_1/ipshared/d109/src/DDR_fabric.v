...
module DDR_fabric
    ...
    (* X_INTERFACE_PARAMETER = "FREQ_HZ 50" *)
...
//register a new posedge bit into a unstable register
        if (!aresetn) begin
            ...
            posedge_first <= 1'b0;
            pos_bit_counter <= 0;
            pos_tvalid_high_counter <= 2*(clockratio -1)-1; //3'b1;
            mux_select_pos <= 1'b0;
            pos_tdata_latched <= 0;
            ...
            if (((pos_bit_counter == (msglen/2)) & posedge_first)) begin  //if we started on a posedge, this is the time when the whole frame of data is shifted in already
             ...
             if (tvalid)begin
                if (pos_tvalid_high_counter)
                    ...
                    pos_tvalid_high_counter <= 2*(clockratio -1)-1;
                    pos_tvalid <= 1'b0;
                    pos_tlast <= 1'b0;
                    mux_select_pos <= 1'b0;
                end
...
//register a new posedge bit into a unstable register
        if (!aresetn) begin
           // previous_enable_neg <= 0;
            ...
            negedge_first <= 1'b0;
            neg_bit_counter <= 0;
            neg_tvalid_high_counter <= 2*(clockratio -1)-1;
            mux_select_neg <= 1'b0;
            neg_tdata_latched <= 0;
