...
void executeCommand(uint8_t *tx_buffer) {
	...
	uint32_t second_bit = 2;

	/* Variable to store memory read data */
	uint32_t mem_read_data;

	/* GPX2 variables */
	uint8_t gpx2_read_data;
	uint32_t gpx2_tdcdata;
	uint8_t gpx2_intr_status;
	...
	// --------------------------------------------------------------------------------- //
	// 32-bit read from memory location
	// Field 1 = address
	// --------------------------------------------------------------------------------- //
	case READ_WORD:
	...
	// --------------------------------------------------------------------------------- //
	// GPX2_WRITE_BYTE: Write to GPX2 Register
	// Field 1 = address ; Field 2 = data
	// --------------------------------------------------------------------------------- //
	case GPX2_WRITE_BYTE:
		/* Write the data and update the response buffer */
		//axiGpOutClear(SPI_SEL_1); //ps spi module

		gpx2_WriteByte((uint8_t) field1, (uint8_t) field2);
		setResponseBytes(tx_buffer, GPX2_WRITE_BYTE_OKAY);
		//axiGpOutSet(SPI_SEL_1);		//pl spi module

		break;


	// --------------------------------------------------------------------------------- //
	// GPX2_READ_BYTE: Read from GPX2 Register
	// Field 1 = GPX2 register address
	// --------------------------------------------------------------------------------- //
	case GPX2_READ_BYTE:
		/* Write the data and update the response buffer */
		...
		gpx2_read_data = gpx2_ReadByte((uint8_t) field1);
		setResponseBytes(tx_buffer, (uint32_t) gpx2_read_data);
		//axiGpOutSet(SPI_SEL_1);		//pl spi module
	...
	// --------------------------------------------------------------------------------- //
	// GPX2_READ_TDCDATA: Read back GPX2 TDC-Data
	// Field 1 = n/a, Field 2 = n/a
	// --------------------------------------------------------------------------------- //
	case GPX2_READ_TDCDATA:
		/* Write the data and update the response buffer */
		gpx2_tdcdata = gpx2_ReadTDCData();
		setResponseBytes(tx_buffer, gpx2_tdcdata);
		break;


	case GPX2_INIT_START:
		/* initialize gpx2 with the updated configuration registers */
	...
	case GPX2_RESULT_CHANNEL_SELECT:
		//set the axi output to the value requested
		if (field1&&second_bit){
		...
		setResponseBytes(tx_buffer, GPX2_SPI_CHANNEL_SELECT_OKAY);
		break;
